...
//still wrong - clear doubt 
vector<pii> vec_to_sort(mp.begin(),mp.end());
sort(vec_to_sort.begin(),vec_to_sort.end(),compare);

fo(i,vec_to_sort.size()){
    int count = vec_to_sort[i].second;
    while(count--){
    cout<<vec_to_sort[i].first<<" ";
}
}
return (-1);
//base condition  
